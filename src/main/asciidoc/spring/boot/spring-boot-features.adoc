...
(Spring Boot에서 Cassandra를 사용하는 방법에 대한 자세한 내용은 이 장 앞부분의 "<<boot-features-cassandra>>" 를 참조하세요.)
...
기본적으로 data JPA 테스트는 transactional이며 각 테스트가 끝날 때 roll back됩니다.
...
기본적으로 JDBC 테스트는 transactional이며 각 테스트가 끝날 때 roll back됩니다.
...
("<<boot-features-testing-spring-boot-applications-testing-autoconfigured-jpa-test>>" 참고.)
