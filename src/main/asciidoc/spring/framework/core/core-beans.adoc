...
이 chapter서는 Spring의 Inversion of Control(IoC) container를 다룹니다.
...
이 chapter에서는 Inversion of Control(IoC) 원칙의 Spring Framework 구현을 다룹니다.
IoC는 dependency injection(DI)라고도 합니다.
object가 constructor argument, factory method에 대한 argument 또는 factory method에서 생성되거나 반환된 후 object instance에 설정된 property를 통해서만 개체가 dependency(즉, 작업하는 다른 object)를 정의하는 프로세스입니다.
그런 다음 container는 bean을 생성할 때 이러한 dependency를 주입합니다.
이 process는 기본적으로 class의 direct construction 또는 Service Locator pattern과 같은 mechanism을 사용하여 dependency의 instance화 또는 location을 제어하는 bean 자체의 역(이런 이유로 이름이 Inversion of Control)입니다.
